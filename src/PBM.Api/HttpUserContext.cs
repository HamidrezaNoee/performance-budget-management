...
public sealed class HttpUserContext(IHttpContextAccessor accessor) : IUserContext
{
    ...
    public IReadOnlySet<Guid> WritableCompanyIds => Principal.FindAll("company_write_id").Select(x => Guid.TryParse(x.Value, out var id) ? id : Guid.Empty).Where(x => x != Guid.Empty).ToHashSet();
