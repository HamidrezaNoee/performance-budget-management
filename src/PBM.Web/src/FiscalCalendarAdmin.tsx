...
export default function FiscalCalendarAdmin({ companyId }: { companyId: string }) {
  ...
  const createYear = async () => {
    ...
    try {
      ...
      setCode(''); setName(''); await reload(); notifyWorkspaceChanged()
  ...
  const setYearClosed = async (year: FiscalYear, isClosed: boolean) => {
    ...
    setBusy(true); setError('')
    try { await api.put(`/admin/fiscal-calendar/years/${year.id}/closed`, { isClosed }); await reload(); notifyWorkspaceChanged() }
  ...
  const suggestedYear = jalaliYear || currentJalaliYear()

  return <Stack spacing={2.5}>
    {error && <Alert severity="error">{error}</Alert>}
    <Card elevation={0}><CardContent>
      <Typography variant="h6" fontWeight={900}>تعریف سال مالی و دوره‌های شمسی</Typography>
      <Typography color="text.secondary" mb={2}>برای هر شرکت می‌توانید چند سال مالی مستقل تعریف کنید. شروع سال مالی نیز می‌تواند هر ماهی باشد؛ تاریخ‌ها برای ذخیره‌سازی به میلادی تبدیل می‌شوند.</Typography>
      <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1.5}>
        <TextField size="small" label="کد سال مالی" value={code} onChange={e => setCode(e.target.value)} placeholder={`FY-${suggestedYear}`} />
        <TextField size="small" label="نام سال مالی" value={name} onChange={e => setName(e.target.value)} placeholder={`سال مالی ${suggestedYear}`} />
      ...
      <Alert severity="info" sx={{ mt: 2 }}>برای رفتن از یک سال مالی به سال دیگر، سال جدید را یک‌بار ایجاد کنید؛ سپس از انتخابگر «سال مالی» در بالای سامانه بین سال‌ها جابه‌جا شوید.</Alert>
    </CardContent></Card>
