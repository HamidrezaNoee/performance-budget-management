...
export default function SalesPlanner({ companyId, fiscalYearId, canWrite }: { companyId: string; fiscalYearId: string; canWrite: boolean }) {
  ...
  const [selections, setSelections] = useState<Record<string, string>>({}); const [valueKind, setValueKind] = useState<0 | 3>(0); const [data, setData] = useState<Data | null>(null)
  ...
  const query = async () => {
    ...
    catch (e) { setError(apiError(e, 'بارگذاری بودجه/Forecast فروش ناموفق بود.')) } finally { setBusy(false) }
  }
  ...
  return <Stack spacing={2.5}>
    ...
    <Card elevation={0} sx={{ background: 'linear-gradient(135deg, rgba(2,132,199,.08), rgba(124,58,237,.06))' }}><CardContent><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2} alignItems={{ md: 'center' }}><Box><Stack direction="row" spacing={1} alignItems="center"><PointOfSaleRoundedIcon color="primary" /><Typography variant="h5" fontWeight={900}>بودجه و پیش‌بینی چندبعدی فروش</Typography></Stack><Typography color="text.secondary" mt={1}>جزئیات ماهانه مشابه فایل بودجه: تعداد، آفر، نرخ، فروش ناخالص، تخفیف ریالی و جنسی، فروش خالص و بهای تمام‌شده عادی/جایزه.</Typography></Box><Button startIcon={<RefreshRoundedIcon />} onClick={() => void loadSetup()} disabled={busy}>بازخوانی</Button></Stack></CardContent></Card>
