...
export default function App() {
  ...
  if (!token) return <Login onLoggedIn={(x) => {
    localStorage.setItem('pbm_token', x.accessToken); localStorage.setItem('pbm_display_name', x.displayName)
    setAccessToken(x.accessToken); setToken(x.accessToken); setDisplayName(x.displayName)
  }} />

  setAccessToken(token)
  return <Dashboard displayName={displayName} onLogout={logout} />
}
...
function Dashboard({ displayName, onLogout }: { displayName: string; onLogout: () => void }) {
  ...
  useEffect(() => {
    api.get<Company[]>('/companies').then(r => {
      setCompanies(r.data); if (r.data.length) setCompanyId(r.data[0].id)
  ...
  useEffect(() => {
    ...
    api.get<FiscalYear[]>('/reference/fiscal-years', { params: { companyId } }).then(r => {
      setYears(r.data); if (r.data.length) setYearId(r.data[0].id)
  ...
  useEffect(() => {
    ...
    api.get<DashboardSummary>('/dashboard/summary', { params: { companyId, fiscalYearId: yearId } })
      .then(r => setSummary(r.data)).catch(() => setError('بارگذاری داشبورد ناموفق بود.')).finally(() => setLoading(false))
  }, [companyId, yearId])
  ...
  const menu = [
    ...
    ['عملکرد و KPI', <InsightsRoundedIcon />], ['پیش‌بینی', <AutoGraphRoundedIcon />],
  ...
  return <Box sx={{ display: 'flex', minHeight: '100vh' }}>
    ...
    <Drawer variant="permanent" anchor="right" sx={{ width: drawerWidth, flexShrink: 0, '& .MuiDrawer-paper': { width: drawerWidth, boxSizing: 'border-box', bgcolor: '#0b2038', color: '#dce8f7', border: 0 } }}>
      ...
      <List sx={{ px: 1 }}>{menu.map(([label, icon], index) => <ListItemButton key={label} selected={index === 0} sx={{ borderRadius: 2, mb: .5, '&.Mui-selected': { bgcolor: 'rgba(56,139,253,.18)' } }}><ListItemIcon sx={{ color: 'inherit', minWidth: 40 }}>{icon}</ListItemIcon><ListItemText primary={label} /></ListItemButton>)}</List>
      <Box flexGrow={1} />
    ...
    <Box component="main" sx={{ flexGrow: 1, pt: 11, pb: 5 }}>
      <Container maxWidth="xl">
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'center' }} spacing={2} mb={3}>
          <Box><Typography variant="h4" fontWeight={900}>داشبورد مدیریت بودجه</Typography><Typography color="text.secondary">{selectedCompany?.name ?? 'انتخاب شرکت'} — نمای لحظه‌ای بودجه، عملکرد و پیش‌بینی</Typography></Box>
        ...
        {!loading && summary && <>
          ...
          <Card elevation={0} sx={{ mt: 3 }}><CardContent sx={{ p: 3 }}><Typography variant="h6" fontWeight={800} mb={2}>روند ماهانه بودجه و عملکرد</Typography><Box sx={{ height: 390, direction: 'ltr' }}><ResponsiveContainer width="100%" height="100%"><BarChart data={summary.monthly}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="periodName" /><YAxis tickFormatter={v => `${Math.round(Number(v) / 1_000_000_000)}B`} /><Tooltip formatter={(v) => formatAmount(Number(v))} /><Legend /><Bar dataKey="budget" name="بودجه" fill="#0b5cad" radius={[6,6,0,0]} /><Bar dataKey="actual" name="عملکرد" fill="#00a6a6" radius={[6,6,0,0]} /><Line type="monotone" dataKey="forecast" name="پیش‌بینی" stroke="#ef8c22" strokeWidth={3} dot={false} /></BarChart></ResponsiveContainer></Box></CardContent></Card>
        </>}
      </Container>
    </Box>
  </Box>
