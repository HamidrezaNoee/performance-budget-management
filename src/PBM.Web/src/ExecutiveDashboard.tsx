...
import PortfolioDimensionPanel from './PortfolioDimensionPanel'
...
export default function ExecutiveDashboard({ companyId, fiscalYearId }: { companyId: string; fiscalYearId: string }) {
  ...
  const commercialPanels = <>
    ...
    <PortfolioDimensionPanel companyId={companyId} fiscalYearId={fiscalYearId} />
  ...
  return <Stack spacing={3}>
    ...
    <Card elevation={0}><CardContent><Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }} justifyContent="space-between"><Box><Typography variant="h6" fontWeight={900}>شاخص داشبورد اجرایی</Typography><Typography color="text.secondary" variant="body2">کارت‌ها، روند ماهانه و Drill-down عمومی بر اساس یک Measure واحد هستند؛ پنل‌های Portfolio، تحلیل Dimensionهای گروه، خرید، فروش و هزینه در بالا گزارش‌های دامنه‌ای مستقل‌اند.</Typography></Box><FormControl size="small" sx={{ minWidth: 280 }}><InputLabel>شاخص مالی</InputLabel><Select value={metricCode} label="شاخص مالی" onChange={e => setMetricCode(e.target.value)}>{metrics.map(m => <MenuItem key={m.code} value={m.code}>{m.name} ({m.code})</MenuItem>)}</Select></FormControl></Stack>{selectedMetric && <Typography variant="caption" color="text.secondary" display="block" mt={1.5}>واحد: {selectedMetric.unit || 'مبلغ'} — ارز پایه: {selectedMetric.currencyCode}</Typography>}</CardContent></Card>
