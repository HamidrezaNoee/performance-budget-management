...
export default function ExecutiveDashboard({ companyId, fiscalYearId }: { companyId: string; fiscalYearId: string }) {
  ...
  if (loadingMetrics) return <Stack spacing={3}>{companyPanels}<Box display="flex" justifyContent="center" py={8}><CircularProgress /></Box></Stack>
  if (error && !metrics.length) return <Stack spacing={3}>{companyPanels}<Alert severity="error">{error}</Alert></Stack>
  if (!metrics.length) return <Stack spacing={3}>{companyPanels}<Alert severity="info">برای شرکت و سال مالی انتخاب‌شده شاخص مبلغی عمومی قابل نمایش وجود ندارد.</Alert></Stack>

  return <Stack spacing={3}>
    {companyPanels}
    {error && <Alert severity="error">{error}</Alert>}
    <Card elevation={0}><CardContent><Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }} justifyContent="space-between"><Box><Typography variant="h6" fontWeight={900}>شاخص داشبورد اجرایی</Typography><Typography color="text.secondary" variant="body2">کارت‌ها، روند ماهانه و Drill-down فقط بر اساس اطلاعات همین شرکت و سال مالی انتخاب‌شده نمایش داده می‌شوند. داشبوردهای خرید، فروش و هزینه نیز فقط در محدوده همین شرکت هستند.</Typography></Box><FormControl size="small" sx={{ minWidth: 280 }}><InputLabel>شاخص مالی</InputLabel><Select value={metricCode} label="شاخص مالی" onChange={e => setMetricCode(e.target.value)}>{metrics.map(m => <MenuItem key={m.code} value={m.code}>{m.name} ({m.code})</MenuItem>)}</Select></FormControl></Stack>{selectedMetric && <Typography variant="caption" color="text.secondary" display="block" mt={1.5}>واحد: {selectedMetric.unit || 'مبلغ'} — ارز پایه: {selectedMetric.currencyCode}</Typography>}</CardContent></Card>
