...
export default function ReferenceAdmin({ companyId }: { companyId: string }) {
  ...
  const reload = async () => {
    setError('')
    try {
      const [currencyResponse, sourceResponse, rateResponse, auditResponse] = await Promise.all([
        api.get<Currency[]>('/reference/currencies'), api.get<Source[]>('/reference/fx-rate-sources'), api.get<FxRate[]>('/reference/fx-rates'), api.get<Audit[]>('/audit/recent', { params: { take: 200 } })
      ])
      setCurrencies(currencyResponse.data); setSources(sourceResponse.data); setRates(rateResponse.data); setAudit(auditResponse.data)
  ...
  const saveRate = async () => {
    ...
    } catch { setError('ثبت نرخ ارز ناموفق بود.') }
  }
