...
export default function ReferenceAdmin({ companyId, roles }: { companyId: string; roles: string[] }) {
  ...
  return <Stack spacing={2.5}>
    <Card elevation={0}><Tabs value={tab} onChange={(_, value) => setTab(value)} variant="scrollable" scrollButtons="auto"><Tab label="ارز و نرخ ارز" /><Tab label="تقویم مالی" /><Tab label="شرکت و ساختار سازمانی" disabled={!canManageSecurity} /><Tab label="کاربران و دسترسی" disabled={!canManageSecurity} /><Tab label="تاریخچه تغییرات" disabled={!canViewAudit} /></Tabs></Card>
    {error && tab !== 3 && <Alert severity="error">{error}</Alert>}
    {tab === 0 && <>
