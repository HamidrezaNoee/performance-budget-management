...
export default function ExpensePlanner({ companyId, fiscalYearId, canWrite }: { companyId: string; fiscalYearId: string; canWrite: boolean }) {
  ...
  const [valueKind, setValueKind] = useState<PlanningKind>(0)
  ...
  const query = async () => {
    ...
    } catch (e) { setError(message(e, 'بارگذاری Budget / Actual / Forecast هزینه‌های ماهانه ناموفق بود.')) }
    finally { setBusy(false) }
  }
  ...
  return <Stack spacing={2.5}>
    ...
    <Card elevation={0} sx={{ background: 'linear-gradient(135deg, rgba(124,58,237,.07), rgba(234,88,12,.06))' }}><CardContent>
      <Stack direction="row" spacing={1} alignItems="center"><PaymentsRoundedIcon color="primary" /><Typography variant="h5" fontWeight={900}>Budget / Actual / Forecast هزینه‌ها و مراکز هزینه</Typography></Stack>
      <Typography color="text.secondary" mt={1}>حقوق و مزایا، اداری، بازاریابی، فروش، سایر عملیاتی، مالی و غیرعملیاتی به تفکیک مرکز هزینه و سایر Dimensionها؛ Actual فقط خواندنی است.</Typography>
    </CardContent></Card>
    ...
    {setup && <Card elevation={0}><CardContent>
      <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1.25} flexWrap="wrap" useFlexGap>
        <ToggleButtonGroup exclusive size="small" value={valueKind} onChange={(_, v: PlanningKind | null) => v !== null && setValueKind(v)}><ToggleButton value={0}>بودجه</ToggleButton><ToggleButton value={1}>عملکرد واقعی</ToggleButton><ToggleButton value={3}>Forecast</ToggleButton></ToggleButtonGroup>
      ...
      {version && !isActual && !editable && <Alert severity="warning" sx={{ mt: 2 }}>نسخه انتخابی قابل ویرایش نیست.</Alert>}
    ...
    {data && <>
      ...
      <Card elevation={0}><CardContent sx={{ p: 0 }}><Box p={2.5}><Typography variant="h6" fontWeight={900}>جزئیات ماهانه</Typography></Box><TableContainer><Table size="small"><TableHead><TableRow><TableCell>ماه</TableCell><TableCell>مبلغ</TableCell><TableCell>سهم از سال</TableCell></TableRow></TableHead><TableBody>{data.periods.map(p => {
        ...
        return <TableRow key={p.id} hover><TableCell><Typography fontWeight={800}>{p.name}</Typography></TableCell><TableCell sx={{ width: 300 }}>{isActual ? <Typography fontWeight={700}>{number.format(current)}</Typography> : <TextField size="small" type="number" value={current} disabled={!editable || p.isClosed || saving === p.id} onChange={e => setData(x => x ? { ...x, values: x.values.map(v => v.periodId === p.id ? { ...v, value: Number(e.target.value) } : v) } : x)} onBlur={e => void save(p.id, e.target.value)} inputProps={{ min: 0, step: 'any' }} />}</TableCell><TableCell>{annual === 0 ? '۰٪' : `${number.format(Number(current) / annual * 100)}٪`}</TableCell></TableRow>
