...
public sealed class WorkbookNormalizationService : IWorkbookNormalizationService
{
    ...
    private static void NormalizeDepartmentExpense(string sheetName, IReadOnlyList<SheetRow> rows, ICollection<NormalizedWorkbookFactDto> facts, ICollection<string> warnings)
    {
        ...
        foreach (var row in rows.Where(x => x.Index > header.Index))
        {
            ...
            foreach (var (column, month) in monthColumns)
            {
                ...
                facts.Add(new NormalizedWorkbookFactDto(row.Index, "EXPENSE_AMOUNT", ValueKind.Budget, month, raw * 1_000_000m, "IRR", 1_000_000m,
    ...
    private static void NormalizeFinancialStatement(IReadOnlyList<SheetRow> rows, ICollection<NormalizedWorkbookFactDto> facts, ICollection<string> warnings, FinancialStatementKind kind, int firstLabelColumn, int? secondLabelColumn)
    {
        ...
        NormalizeFinancialStatementSide(rows, header, firstLabelColumn, secondLabelColumn is null ? int.MaxValue : secondLabelColumn.Value - 1, kind, facts);
        if (secondLabelColumn.HasValue) NormalizeFinancialStatementSide(rows, header, secondLabelColumn.Value, int.MaxValue, kind, facts);
    ...
    private static void NormalizeFinancialStatementSide(IReadOnlyList<SheetRow> rows, SheetRow header, int labelColumn, int maxColumn, FinancialStatementKind kind, ICollection<NormalizedWorkbookFactDto> facts)
    {
        ...
        foreach (var row in rows.Where(x => x.Index > header.Index))
        {
            ...
            {
                if (!TryDecimal(row.Get(column), out var raw)) continue;
                facts.Add(new NormalizedWorkbookFactDto(row.Index, "STATEMENT_AMOUNT", ValueKind.Budget, month, raw * 1_000_000m, "IRR", 1_000_000m,
