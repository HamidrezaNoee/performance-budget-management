...
public sealed class FinancialReportService(PbmDbContext db, IUserContext user) : IFinancialReportService
{
    ...
    private static readonly IReadOnlyList<(string Code, string Name)> ProfitLossRows = new[]
    {
        ...
        ("COGS", "قیمت تمام شده کالای تجاری فروش رفته"), ("GROSS_PROFIT", "سود (زیان) ناخالص"),
        ("ADMIN_EXPENSE", "هزینه‌های اداری، عمومی و فروش"), ("OPERATING_PROFIT", "سود (زیان) عملیاتی"),
        ("FINANCE_COST", "هزینه‌های مالی"), ("PROFIT_BEFORE_TAX", "سود (زیان) قبل از مالیات"),
    ...
    public async Task<FinancialReportDto> GetAsync(Guid companyId, Guid fiscalYearId, FinancialReportType type, ValueKind valueKind = ValueKind.Budget, Guid? versionId = null, CancellationToken cancellationToken = default)
    {
        ...
        var template = type switch { FinancialReportType.BalanceSheet => BalanceSheetRows, FinancialReportType.CashFlow => CashFlowRows, _ => ProfitLossRows };
        ...
        var accountMemberCandidates = await db.DimensionMembers.AsNoTracking()
            .Where(x => x.DimensionId == accountDimensionId && (x.CompanyId == null || x.CompanyId == companyId) && codes.Contains(x.Code))
            .Select(x => new { x.Id, x.Code, x.Name, x.CompanyId }).ToListAsync(cancellationToken);
        var accountMembers = accountMemberCandidates.GroupBy(x => x.Code, StringComparer.OrdinalIgnoreCase)
            .Select(g => g.OrderByDescending(x => x.CompanyId == companyId).First()).ToList();
        ...
        var facts = await db.BudgetFacts.AsNoTracking().Where(x => x.VersionId == version.Id && x.MeasureId == statementMeasureId && x.ValueKind == valueKind)
            .Select(x => new { x.PeriodId, x.Value, AccountMemberId = x.Dimensions.Where(d => d.DimensionId == accountDimensionId).Select(d => (Guid?)d.MemberId).FirstOrDefault() })
            .Where(x => x.AccountMemberId.HasValue).ToListAsync(cancellationToken);
        var values = facts.Where(x => byMember.ContainsKey(x.AccountMemberId!.Value)).GroupBy(x => (x.AccountMemberId!.Value, x.PeriodId)).ToDictionary(g => g.Key, g => g.Sum(x => x.Value));
        ...
        {
            var item = template[index]; memberByCode.TryGetValue(item.Code, out var member);
            var cells = periods.Select(p => new FinancialReportCellDto(p.Id, p.Name, p.Sequence, member is not null && values.TryGetValue((member.Id, p.Id), out var value) ? value : 0m)).ToList();
    ...
    private static FinancialReportDto Empty(FinancialReportType type, Guid companyId, Guid fiscalYearId, ValueKind valueKind, IReadOnlyList<PeriodRef> periods, IReadOnlyList<(string Code, string Name)> template) =>
        new(type, companyId, fiscalYearId, null, null, valueKind, template.Select((item, index) => new FinancialReportRowDto(item.Code, item.Name, index + 1, periods.Select(p => new FinancialReportCellDto(p.Id, p.Name, p.Sequence, 0m)).ToList(), 0m)).ToList());

    private async Task EnsureCompanyAsync(Guid companyId, CancellationToken cancellationToken)
    {
        if (!await db.Companies.AnyAsync(x => x.Id == companyId && x.TenantId == user.TenantId, cancellationToken)) throw new UnauthorizedAccessException("Company is outside the current tenant.");
